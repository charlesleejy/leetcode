...
WITH passenger_with_rank AS (
SELECT  passenger_id, flight_id, ROW_NUMBER() OVER (
        PARTITION BY flight_id
		ORDER BY booking_time
	) AS rk FROM Passengers),

joined_data AS (
	SELECT passenger_id, rk,  COALESCE(capacity, 0) AS capacity FROM passenger_with_rank a LEFT JOIN Flights b 
...
ELSE 'Waitlist' 
END AS Status
FROM joined_data ORDER BY passenger_id