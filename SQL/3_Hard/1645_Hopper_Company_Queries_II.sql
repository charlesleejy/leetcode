...
-- T2: The number of drivers during each month listed in Drivers table
T2 AS (
    SELECT 
        IF(YEAR(join_date) < '2020', '1', MONTH(join_date)) AS join_month, 
        COUNT(driver_id) AS driver_num
    FROM Drivers
    WHERE YEAR(join_date) <= '2020'
    GROUP BY join_month
),
-- T3: The number of accepted rides during each month listed in Rides table
T3 AS (
    SELECT 
        MONTH(requested_at) AS join_month, 
        COUNT(DISTINCT ar.driver_id) AS ride_num
...
SELECT 
    T1.join_month AS month, 
    ROUND(100 * IFNULL(T3.ride_num / SUM(T2.driver_num), 0), 2) AS working_percentage
FROM T1
LEFT JOIN T2 ON T1.join_month >= T2.join_month
LEFT JOIN T3 ON T1.join_month = T3.join_month
GROUP BY T1.join_month
ORDER BY T1.join_month;